"""Structured contract clause detection chain."""
...
PROMPT_PATH = Path(__file__).resolve().parents[1] / "prompts" / "clause_detection.txt"


class ClauseDetectionChain:
    def __init__(self, llm: LLMService, *, prompt_path: Path = PROMPT_PATH) -> None:
        ...
        if "{{CONTRACT_TEXT}}" not in self.prompt_template:
            raise ValueError("Clause detection prompt is missing its text placeholder")

    def run(self, chunks: list[ContractChunk]) -> ClauseDetection:
        if not chunks:
            raise ValueError("Contract does not contain text chunks")
        prompt = self.prompt_template.replace(
            "{{CONTRACT_TEXT}}", format_contract_chunks(chunks)
        )
        return self.llm.generate_structured(prompt, ClauseDetection)
